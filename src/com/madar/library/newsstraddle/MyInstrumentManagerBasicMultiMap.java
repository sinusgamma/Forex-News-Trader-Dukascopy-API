...
public class MyInstrumentManagerBasicMultiMap extends MyInstrumentManager{
    ...
    private HashMap<IOrder, MyOrderScenario> detailedOrderMap;
    ...
    // CONSTRUCTOR
    public MyInstrumentManagerBasicMultiMap(Instrument managedInstrument, Long actualEventTime, MyEventType actualEventType, IContext context) {
        ...
        this.detailedOrderMap = new HashMap<>();
        this.orderCounter = 0;
    ...
    public void manageInstrumentOnTick(MyEventState eventState, ITick tick) {
        ...
        if(detailedOrderMap.isEmpty() == false) { // no meaning to modify not existing orders           
                                   
            // MAIN FOR IN TICK - Modify orders if needed in every tick
            for (Iterator<Entry<IOrder, MyOrderScenario>> it = detailedOrderMap.entrySet().iterator(); it.hasNext();) {
                Map.Entry<IOrder, MyOrderScenario> detailedOrder = it.next();
                
                IOrder order = detailedOrder.getKey();
                MyOrderScenario scenario = detailedOrder.getValue();
        ...
        try {
            console.getOut().println("Order Count in " + managedInstrument + " list: " + detailedOrderMap.size() + " - Active Instrument Orders: " + engine.getOrders(managedInstrument));
        } catch (JFException ex) {
            Logger.getLogger(MyInstrumentManagerBasicMultiMap.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void openPositions(MyEventState eventState, ITick tick) {
        ...
        if(eventState != MyEventState.BEFORE || triggeredOpen == true || !detailedOrderMap.isEmpty()) return;
    ...
    private void openPending(MyOrderDirection orderDirection, ITick tick) {
        ...
        for (MyOrderScenario scenario : directionScenarios.get(orderDirection)) {
            ...
            if(order != null){  
            detailedOrderMap.put(order, scenario);
            }
    ...
    private boolean removeInactiveOrder(IOrder order){
        boolean deletedOrder = false;
        if(order == null) {return deletedOrder;} // if no order go back      
        if(detailedOrderMap.containsKey(order) == false){return deletedOrder;} // if order isn't in this.detailedOrderMap go back
        
        if(order.getState() == IOrder.State.CLOSED || order.getState() == IOrder.State.CANCELED){
            detailedOrderMap.remove(order);
            deletedOrder = true;
        }
        return deletedOrder;
    }
    ...
    private void orderCancelOrder(IOrder order) {
        ...
        if(detailedOrderMap.containsKey(order) == false){return;} // if no order or order isn't in this.detailedOrderMap go back       
        if(order.getState() != IOrder.State.FILLED){return;} // we want a FILLED order to trigger OCO
        boolean orderIsLong = order.isLong();
        
        for(HashMap.Entry<IOrder, MyOrderScenario> detailedOrder : detailedOrderMap.entrySet()) {   
            IOrder otherOrder = detailedOrder.getKey();             
            if(otherOrder.isLong() != orderIsLong){
                try {
                    otherOrder.close(); // here we tell the servere to close,but don't remove from map, because if the server can1t close we have to manage it
                    triggeredOCO = true;
                } catch (JFException ex) {
                    Logger.getLogger(MyInstrumentManagerBasicMultiMap.class.getName()).log(Level.SEVERE, null, ex);
